...
export default function Dashboard() {
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true); await fetchPosts(); await fetchUserSession(); setRefreshing(false);
  }, []);
  ...
  const handlePostOptions = (post) => {
    Alert.alert("Post Options", "What do you want to do with this post?", [
        ...
        { text: "Archive", onPress: async () => {
            ...
            fetchPosts(); Alert.alert("Archived", "Post hidden from feed.");
        }},
        { text: "Delete", style: 'destructive', onPress: async () => {
            await supabase.from('posts').delete().eq('id', post.id);
            fetchPosts(); Alert.alert("Deleted", "Post has been removed permanently.");
        }},
  ...
  const handlePostSubmit = async () => {
    ...
    if (editingPostId) {
        ...
        await supabase.from('posts').insert([{ ...postData, likes: 0, comments: 0 }]);
    }
                                  ...
                                  <View style={{flex: 1}}>
                                      ...
                                      <View style={{flexDirection: 'row', alignItems: 'center', marginTop: 5, paddingLeft: 10, gap: 15}}>
                                          ...
                                          <TouchableOpacity onPress={() => handleCommentLike(reply)} style={{flexDirection: 'row', alignItems: 'center', gap: 4}}>
                                              <MaterialCommunityIcons name={reply.liked_by?.includes(userData.name) ? "heart" : "heart-outline"} size={14} color={reply.liked_by?.includes(userData.name) ? "#FF1744" : "#666"} />
                                              <Text style={{fontSize: 12, color: reply.liked_by?.includes(userData.name) ? '#FF1744' : '#666'}}>{reply.likes || 0}</Text>
                                          </TouchableOpacity>
                                      </View>
                                  </View>
                              </View>
          ...
          <View style={{backgroundColor: 'white', borderTopWidth: 1, borderTopColor: '#eee'}}>
              ...
              {replyingTo && (
                  <View style={{backgroundColor: '#E8F5E9', padding: 8, paddingHorizontal: 15, flexDirection: 'row', justifyContent: 'space-between'}}>
                      ...
                      <TouchableOpacity onPress={() => setReplyingTo(null)}><MaterialCommunityIcons name="close" size={16} color="#666" /></TouchableOpacity>
                  </View>
              )}
  ...
  if (isCreating) {
      return (
        <View style={{flex: 1, backgroundColor: 'white'}}>
            ...
            <View style={[styles.createHeader, { paddingTop: Math.max(insets.top, 20) + 15 }]}>
                <TouchableOpacity onPress={() => {setIsCreating(false); setEditingPostId(null);}}><Ionicons name="arrow-back" size={24} color="#333" /></TouchableOpacity>
                <View><Text style={styles.createHeaderTitle}>{editingPostId ? 'Edit Post' : 'Create Post'}</Text></View>
                <View style={{width: 24}} /> 
            </View>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        ...
        <View style={styles.sectionHeader}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                <TouchableOpacity style={styles.notifIconBtn} onPress={() => router.push('/notifications')}><Ionicons name="notifications-outline" size={22} color="#333" /></TouchableOpacity>
...
const ImpactCard = ({ value, unit, icon, color, bgColor }) => (
  <View style={styles.impactCard}><View style={[styles.impactIconBg, { backgroundColor: bgColor }]}><MaterialCommunityIcons name={icon} size={24} color={color} /></View><Text style={[styles.impactValue, { color: color }]}>{value}</Text><Text style={styles.impactUnit}>{unit}</Text></View>
);
