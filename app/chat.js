...
export default function ChatScreen() {
  ...
  return (
    ...
    >
      {/* 🟢 FIXED: Translucent={false} para hindi mag-white gap ang Android keyboard,
                   tapos ibinalik ang padding sa View sa baba para hindi matakpan ang oras/battery */}
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => {
          ...
          const accent = inquiryContext ? getInquiryAccent(inquiryContext.type) : null;
          ...
          return (
            <View style={{marginBottom: isSameSenderAsNext && timeDiffNextMins <= 1 ? 2 : 14}}>
                ...
                {inquiryContext && (
                    <TouchableOpacity activeOpacity={0.9} style={[styles.inquiryCard, isMe ? { alignSelf: 'flex-end' } : { alignSelf: 'flex-start' }]}>
                        ...
                        {showBubble ? (
                            <View style={{flexDirection: 'row', alignItems: 'flex-end'}}>
                                ...
                                <TouchableOpacity activeOpacity={0.85} onLongPress={() => setReplyingTo(item)} style={[styles.bubble, isMe ? styles.myBubble : styles.theirBubble, (isThumbsUp || item.image_url) ? {backgroundColor: 'transparent', padding: 0, borderWidth: 0, elevation: 0, shadowOpacity: 0} : null]}>
                                    ...
                                    {isThumbsUp ? (
                                        <MaterialCommunityIcons name="thumb-up" size={45} color="#007C00" style={{marginLeft: isMe ? 0 : 5}} />
